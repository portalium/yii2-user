<?php
return [
    'This username has already been taken.' => 'Kullanıcı adı daha önce alınmıştır.',
    'Default Role' => 'Varsayılan Rol',
    'Admin' => 'Yönetici',
    'Moderator' => 'Düzenleyici',
    'Users' => 'Kullanıcılar',
    'All Users' => 'Tüm Kullanıcılar',
    'Username' => 'Kullanıcı Adı',
    'First Name' => 'Adı',
    'Last Name' => 'Soyadı',
    'Email' => 'E-posta',
    'Create User' => 'Kullanıcı Oluştur',
    'Group Name' => 'Grup Adı',
    'Group Description' => 'Grup Açıklaması',
    'Description' => 'Açıklama',
    'Roles' => 'Roller',
    'Create Role' => 'Rol Oluştur',
    'Name' => 'İsim',
    'Assignments' => 'Görevler',
    'Assignment' => 'Görev',
    'Bulk Assignment' => 'Toplu Atama',
    'Assign' => 'Ekle',
    'Assigned Users' => 'Atanmış Kullanıcılar',
    'Remove' => 'Kaldır',
    'Search for assigned' => 'Atanmışlarda ara',
    'Update' => 'Güncelle',
    'Create' => 'Oluştur',
    'Delete' => 'Sil',
    'Are you sure to delete this item?' => 'Bu öğeyi silmek istediğinizden emin misiniz?',
    'Assigned users' => 'Eklenen kullanıcılar',
    'Available Users' => 'Geçerli Kullanıcılar',
    'Search for available' => 'Geçerli olan için ara',
    'Search' => 'Ara',
    'Reset' => 'Sıfırla',
    'Groups' => 'Gruplar',
    'All Groups' => 'Tüm Gruplar',
    'Create Group' => 'Grup Oluştur',
    'Manage Members' => 'Üyeleri Yönet',
    'Group Members' => 'Grup Üyeleri',
    'Manage Members for Group: {name}' => 'Grup Üyelerini Yönet: {name}',
    'Update Group: {name}' => 'Grubu Güncelle: {name}',
    'Save' => 'Kaydet',
    'There is no user in the group.' => 'Grupta kullanıcı yok.',
    'Users:' => 'Kullanıcılar:',
    'User Import' => 'Kullanıcı Ekle',
    'Create Users' => 'Kullanıcı Oluştur',
    'Update User: ' => 'Kullanıcı Güncelle: ',
    'Update User: ' => 'Kullanıcı Güncelle: ',
    'Are you sure you want to delete this item?' => 'Bu öğeyi silmek istediğinizden emin misiniz?',
    'User does not belong to any group.' => 'Kullanıcı herhangi bir gruba ait değil.',
    'User groups: ' => 'Kullanıcı Grupları: ',
    'Permissions' => 'İzinler',
    'Password' => 'Şifre',
    'Import User' => 'Kullanıcı Aktar',
    'First column index starts from 0'=>'İlk kolonun indeks değeri 0\'dan başlar',
    'First Name Column Name' => 'İsim Sütunu Adı',
    'Lastname Name Column Name' => 'Soyisim Sütunu Adı',
    'Username Column Name' => 'Kullanıcı Adı Sütunu Adı',
    'Email Column Name' => 'E-posta Sütunu Adı',
    'Password Column Name' => 'Şifre Sütunu Adı',
    'The Signup Page feature must be activated in the Settings.' => 'Settings içerisinden Signup Page özelliğinin aktif edilmesi gerekiyor.',
    'Username already exists' => 'Kullanıcı adı zaten var',
    'Email already exists' => 'E-posta zaten var',
    'User Management Module' => 'Kullanıcı Yönetim Modülü',
    'Warning: When the user is deleted, his records in the marked modules will also be deleted. Tick the records you want to be deleted.' => 'Uyarı: Kullanıcı silindiğinde, işaretli modüllerdeki kayıtları silinir. Silmek istediğiniz kayıtları işaretleyin.',
    'Note: Unchecked Records will be assigned to the default user.' => 'Not: İşaretlenmeyen modüllerdeki kayıtlar varsayılan kullanıcıya atanacaktır.',
    'Active' => 'Aktif',
    'Passive' => 'Pasif',
    'Status' => 'Durum',
    'Delete Selected Users' => 'Seçilen Kullanıcıları Sil',
    'If you continue, all your data will be reset. Do you want to continue?' => 'Devam ederseniz, kullanıcıların tüm verileri sıfırlanacaktır. Devam etmek istiyor musunuz?',
    'Created At' => 'Oluşturulma Tarihi',
    'Updated At' => 'Güncellenme Tarihi',
    'User has been deleted' => 'Kullanıcı silindi',
    'User has been updated' => 'Kullanıcı güncellendi',
    'User has been created' => 'Kullanıcı oluşturuldu',
    'Actions'  => 'Eylemler',
    'Assignment' => 'Görevlendirme',
    'Deleted' => 'Silinen',
    'Update User' => 'Kullanıcı Güncelle',
    'Date Create' => 'Oluşturulma Tarihi',
    'Access Token' => 'Erişim Parolası',
    'Actions' => 'Eylemler',
    'Add to Group' => 'Gruba Ekle',
    'Remove from Group' => 'Gruptan Çıkar',
    'Settings saved.' => 'Ayarlar kaydedildi.',
    'There was an error. Settings not saved successfully.' => 'Bir hata oluştu. Ayarlar başarıyla kaydedilemedi.',
    'Group has been deleted.' => 'Grup silindi.',
    'Group has been cleared of users and deleted.' => 'Gruptaki kullanıcılar temizlendi ve silindi.',
    'Error deleting group: ' => 'Grup silinirken hata oluştu: ',
    'The requested page does not exist.' => 'İstenilen sayfa bulunamadı.',
    'You are not allowed to access this page.' => 'Bu sayfaya erişiminiz yetkilendirilmedi.',
    'Sorry, you are not allowed to view this page.' => 'Üzgünüz, bu sayfayı görüntülemenize izin verilmiyor.',
    'An error occurred while deleting the user. Please try again.' => 'Kullanıcı silinirken bir hata oluştu. Lütfen tekrar deneyin.',
    'User could not be deleted after group updates.' => 'Grup güncellemelerinden sonra kullanıcı silinemedi.',
    'User has been removed from groups and deleted.' => 'Kullanıcı gruplardan çıkarıldı ve silindi.',
    'Failed to remove user from group:' => 'Kullanıcı gruptan kaldırılamadı:',
    'User could not be deleted.' => 'Kullanıcı silinemedi.',
    'Please choose file.' => 'Lütfen dosya seçin.',
    'File not found: ' => 'Dosya bulanamadı: ',
    'You entered the username or e-mail columns.' => 'Kullanıcı adı veya e-posta sütunlarını girdiniz.',
    'Username contains invalid characters.' => 'Kullanıcı adı geçersiz karakterler içeriyor.',
    'Date Create' => 'Oluşturma Tarihi',
    'Delete Multiple' => 'Çoklu Sil',
    'User cannot be deleted without deleting menu items!' => 'Menü öğeleri silinmeden kullanıcı silinemez!'
];
